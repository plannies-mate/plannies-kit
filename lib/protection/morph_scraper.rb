...
module Protection
  class MorphScraper
    ...
    def scraper_in_use(authority_url)
      @scraper_in_use ||= {}
      @scraper_in_use[authority_url] ||=
        begin
          url = "#{authority_url}/under_the_hood"
          ...
          page.search("a").each do |link|
            ...
            scraper = match[1]
            puts "Found #{authority_url} uses scraper: #{scraper.inspect}" if ENV["DEBUG"]
            return scraper
          end
    ...
    def fetch_data(authority)
      url = "https://www.planningalerts.org.au/authorities/#{authority}"
      page = Mechanize.new.get(url)
      result = {
      ...
      result[:scraper] = scraper_in_use(url)
      result
